...
AlertTable::AlertTable(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::AlertTable)
{
    ui->setupUi(this);
    initialTables();
    updateData_Test();
...
void AlertTable::addTarget_Test()
{
...
void AlertTable::updateData_Test()
{
    d = new DataTest();
...
void AlertTable::initialHeaders(QStandardItemModel *model, QTableView *tableView)
{
    QStringList headers;
...
void AlertTable::initialSize(QStandardItemModel *model, QTableView *tableView)
{
    if (!QString::compare(tableView->objectName(),"table_infraredTargets"))
    {
...
void AlertTable::initialDelegate(QTableView *tableView)
{
    if (!QString::compare(tableView->objectName(),"table_infraredTargets"))
    {
...
void AlertTable::initialPopMenu()
{
...
void AlertTable::initialEditorValidators()
{
...
void AlertTable::clearAllAreasInput()
{
    ui->lineEdit  ->clear();
    ui->lineEdit_2->clear();
    ui->lineEdit_3->clear();
...
void AlertTable::adjustAreaHeaderNum()
{
...
void AlertTable::removeAllAlertAreas()
{
    // 清表格
...
void AlertTable::removeAllIgnoreAreas()
{
    // 清表格
...
void AlertTable::removeAlertAreaAt(int index)
{
    // 清表格
...
void AlertTable::removeIgnoreAreaAt(int index)
{
    // 清表格
...
bool AlertTable::isDirectionEmpty()
{
...
bool AlertTable::isDirectionValid(double startDirection, double endDirection)
{
...
bool AlertTable::isRadiusValid(double startRadius, double endRadius)
{
...
void AlertTable::initialLineList()
{
    if (lineList.size() == 0)
        for (int i = 0; i < LIST_SIZE; ++i) {
...
void AlertTable::radarDataHandler()
{
...
void AlertTable::uploadPolicyHandler()
{
//    upLoadFlag = UPLOAD_POLICY == "auto"?false:true;
}

//无用
void AlertTable::setTargetValue(QString name, int index, int value)
{
    int targetIndex;
    bool found = false;
...
//无用
QVariant AlertTable::getTargetValue(QString name, int index)
{
    int temp;
    QVariant result;
...
void AlertTable::getRadarIndex(int targetIndex)
{
...
void AlertTable::selectAllTargets(bool state)
{
...
void AlertTable::checkTableModifiable(bool state)
{
    infraredTargetModel->isEnable = state;
    infraredTargetModel->isSelectable = state;
}

void AlertTable::on_btn_addAlertArea_clicked()
{
...
void AlertTable::on_btn_addIgnoreArea_clicked()
{
...
void AlertTable::on_btn_cancelAlertAreas_clicked()
{
    clearAllAreasInput();
    removeAllAlertAreas();
//    adjustAlertArea();
...
void AlertTable::on_btn_cancelIgnoreAreas_clicked()
{
    clearAllAreasInput();
...
void AlertTable::receiveTestData(int data)
{
    ui->lineEdit->setText(QString::number(data));
}

void AlertTable::receiveTestData(Area area)
{
...
void AlertTable::receiveTestData(QVector<SCTarget> infraredList, QVector<SCTarget> radarList)
{
    dataHandler();
    radarDataHandler();
...
void AlertTable::showATarget(int row)
{
...
void AlertTable::receiveTestData(SCTarget target)
{
...
void AlertTable::receive_deleteClicked(QModelIndex index)
{
    //弹出提示框，看是否删除数据
...
void AlertTable::receiveCatagroy(int col, int row, QString text)
{
    int category;
...
void AlertTable::check_changed(QStandardItem *item)
{
    QVariant a;
...
void AlertTable::slot_clickRightMenu(QPoint pos)
{
...
void AlertTable::slot_menuChoiceAction(QAction *act)
{
    //弹出提示框，看是否删除数据
    QMessageBox message(QMessageBox::NoIcon, "提示","是否删除该区域?",QMessageBox::Yes | QMessageBox::No, NULL);
